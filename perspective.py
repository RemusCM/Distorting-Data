...
def verticalPerspective(point, angle):
...
def horizontalPerspective(point, angle):
...
def perspective(file):
        ...
        # #PERSPECTIVE
        scalingFactor = 1
        perspectiveGlobal = random.random()
        ...
        for c in range(0, len(contours)):
            ...
            if(perspectiveType < 0.5):
                for point in contours[c]:
                    horizontalPerspective(point,angle)
                ...
                for point in contours[c]:
                    verticalPerspective(point,angle)
                    if(perspectiveGlobal < 0.5):
                        rotation(point, rotationFactor)
                        scaling(point, scalingFactor)
        ...
        filename = file[:len(file)-4] + "_perspective" + file[len(file)-4:]
        print(filename)
