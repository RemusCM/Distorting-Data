...
def horizontalShrinkRotation(point,angle,rotation_angle):
...
def verticalShrinkRotation(point,angle,rotation_angle):
...
def shrink_rotation(file):
        ...
        for c in range(0, len(contours)):
            ...
            if(shrinkType < 0.5):
                for point in contours[c]:
                    horizontalShrinkRotation(point,angle,rotation_angle)
                ...
                for point in contours[c]:
                    verticalShrinkRotation(point,angle,rotation_angle)
                    if(shrinkGlobal < 0.5):
                        rotation(point, rotationFactor)
                        scaling(point, scalingFactor)
        ...
        filename = file[:len(file)-4] + "_shrink_rotation" + file[len(file)-4:]
        print(filename)
